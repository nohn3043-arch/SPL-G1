...
module spl_pim_cell_v2 #(
    ...
    input  logic [63:0] ra_data_in,
    input  logic        pim_store_en,
    output logic [63:0] ra_data_out,
    ...
    logic [64:0] add_ext;       // 65-bit: {carry, 64-bit sum}
    logic [64:0] sub_ext;       // 65-bit: {borrow, 64-bit diff}
    logic [127:0] mul_full;     // 128-bit full product

    assign add_ext = {1'b0, local_store} + {1'b0, ra_data_in} + {64'b0, carry_reg};
    assign sub_ext = {1'b0, local_store} - {1'b0, ra_data_in} - {64'b0, borrow_reg};
    ...
    always @(*) begin
        ...
        if (exec_active) begin
            case (ra_op)
                ...
                OP_ADD:    alu_result = add_ext[63:0];
                OP_ADC:    alu_result = add_ext[63:0];     // carry_reg sourced from neighbour
                OP_SUB:    alu_result = sub_ext[63:0];
                OP_SBB:    alu_result = sub_ext[63:0];

                // ── Multiply ──
                OP_MUL_LO: alu_result = mul_full[63:0];
                OP_MUL_HI: alu_result = mul_full[127:64];
                OP_MAC:    alu_result = local_store + mul_full[63:0];

                // ── Comparison (result = {63'b0, condition}) ──
                OP_CMP_EQ: alu_result = {63'b0, (local_store == ra_data_in)};
                OP_CMP_NE: alu_result = {63'b0, (local_store != ra_data_in)};
                OP_CMP_LT: alu_result = {63'b0, ($signed(local_store) <  $signed(ra_data_in))};
                OP_CMP_GT: alu_result = {63'b0, ($signed(local_store) >  $signed(ra_data_in))};
                ...
                // Each stub operates on low 16 bits; upper 48 bits zero-extended.
                OP_FP16_ADD: alu_result = {48'h0, fp16_a + fp16_b};
                OP_FP16_SUB: alu_result = {48'h0, fp16_a - fp16_b};
                OP_FP16_MUL: alu_result = {48'h0, fp16_a * fp16_b};
                OP_FP16_CMP: alu_result = {63'b0, (fp16_a == fp16_b)};
                OP_FP16_MAC: alu_result = {48'h0, fp16_a + (fp16_a * fp16_b)};

                // ── Predicate ──
                OP_PRED_SET: alu_result = {63'b0, ra_data_in[0]};  // just passthrough; pred_reg set separately

                // ── Data movement (neighbour → local path) ──
                OP_LOAD_ROW: alu_result = {56'h0, row_data_l};
                OP_LOAD_COL: alu_result = {56'h0, col_data_l};
