...
module spl_pim_compute_array #(
    ...
    output logic [DATA_W-1:0]     vec_sum   [COLS-1:0],
    output logic [DATA_W-1:0]     mat_total,
    ...
    //   cell_state_obs_packed[ROWS*COLS*DATA_W-1:0]
    //   cell_op_obs_packed[ROWS*COLS*5-1:0]
    output logic [ROWS*COLS*DATA_W-1:0] cell_state_obs_packed,
    output logic [ROWS*COLS*5 -1:0] cell_op_obs_packed
);

    logic [DATA_W-1:0] cell_data_out [ROWS-1:0][COLS-1:0];
        ...
        for (r = 0; r < ROWS; r = r + 1) begin : gen_row
            for (c = 0; c < COLS; c = c + 1) begin : gen_col
                ...
                spl_pim_cell #(.IDX_ROW(r), .IDX_COL(c), .DATA_W(DATA_W)) u_cell (
                    .ra_clk, .ra_rst_n,
                    .ra_op        (cell_op),
                    .ra_data_in   (row_sel[r] && col_sel[c] ? ra_wdata : {DATA_W{1'b0}}),
    ...
    logic [DATA_W-1:0] rd_cell [ROWS-1:0][COLS-1:0];
    genvar rr, rc;
    generate
        for (rr = 0; rr < ROWS; rr = rr + 1) begin : gen_rd_r
            for (rc = 0; rc < COLS; rc = rc + 1) begin : gen_rd_c
                assign rd_cell[rr][rc] = (ra_en && exec_mode == 2'b01 &&
                                         ...
                                         ? cell_data_out[rr][rc] : {DATA_W{1'b0}};
            end
        end
    endgenerate
    always_comb begin
        ra_rdata = {DATA_W{1'b0}};
    ...
    generate
        for (mo_r = 0; mo_r < ROWS; mo_r = mo_r + 1) begin : gen_mat_obs_r
            for (mo_c = 0; mo_c < COLS; mo_c = mo_c + 1) begin : gen_mat_obs_c
                assign cell_state_obs_packed[((mo_r*COLS+mo_c)+1)*DATA_W-1 -: DATA_W] = cell_data_out[mo_r][mo_c];
    ...
    logic [DATA_W-1:0] flag_cell_val;
    always_comb begin
        flag_cell_val = {DATA_W{1'b0}};
    ...
    always_ff @(posedge ra_clk or negedge ra_rst_n) begin
        if (!ra_rst_n) begin
            ...
            unique case (exec_mode)
                2'b01:   pim_flag <= (flag_cell_val == {DATA_W{1'b0}});
                2'b10:   pim_flag <= (vec_sum[sel_col_idx] == {DATA_W{1'b0}});
                2'b11:   pim_flag <= (mat_total == {DATA_W{1'b0}});
