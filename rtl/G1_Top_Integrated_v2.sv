// ============================================================================
// G1_Top_Integrated_v2 — SPL-G1 Full Integrated Top (v2.0, Phase 1 complete)
// ============================================================================
// Integrates all v2 components:
//   1. RA-BUS Arbiter           — unified address plane (4 targets, unchanged)
//   2. PIM Compute Array v2     — spl_pim_compute_array_v2 (parameterized, Cell v2)
//   3. PIM Sequencer            — spl_pim_sequencer (unchanged, 16-entry microcode)
//   4. Causal Audit Array v2    — spl_cim_causal_unit_v2 × 4 (NOMOS constraint check)
//   5. Identity Anchor          — 256-bit hash verification (unchanged)
//   6. SBC Sequencer            — orchestrates compute→audit flow (unchanged)
//
// Drop-in replacement for G1_Top_Integrated with identical port map.
// Compatibility: old testbench (tb_G1_Integrated) works with this top
//   — provided the PIM tags don't trigger NOMOS constraint failures.
//   (Bridge mode: constraint_bits = all-1s → audit always passes.)
//
// License: SPL-G1 dual-track (see LICENSE)
// ============================================================================

module G1_Top_Integrated_v2 (
    input  logic         clk,
    input  logic         rst_n,
    ...
    spl_pim_compute_array_v2 #(.ROWS(PIM_ROWS), .COLS(PIM_COLS)) u_pim_array (
    ...
    generate
        ...
        for (gi = 0; gi < 4; gi = gi + 1) begin : g1_audit
            spl_cim_causal_unit_v2 u_causal (
                .clk, .rst_n,
